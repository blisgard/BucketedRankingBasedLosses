...
from mmdet.core import (bbox_cxcywh_to_xyxy, bbox_xyxy_to_cxcywh,
                        ...
                        reduce_mean, vectorize_labels, bbox_overlaps)
...
class RankBasedDETRHead(AnchorFreeHead):
    ...
    def __init__(self,
        ...
        if class_weight is not None and (self.__class__ is RankBasedDETRHead):
        ...
        elif self.rank_loss_type == 'APLoss':
            self.loss_rank = ranking_losses.APLoss()
        elif self.rank_loss_type == 'BucketedAPLoss':
    ...
    def _load_from_state_dict(self, state_dict, prefix, local_metadata, strict,
        ...
        if (version is None or version < 2) and self.__class__ is RankBasedDETRHead:
            convert_dict = {
    ...
    def loss(self,
        ...
        if self.rank_loss_type == 'RankSort' or self.rank_loss_type == 'BucketedRankSort':
            ...
            losses_rank, losses_bbox, losses_iou = multi_apply(
            self.loss_single, all_cls_scores, all_bbox_preds,
            all_gt_bboxes_list, all_gt_labels_list, img_metas_list,
            all_gt_bboxes_ignore_list)
            ...
            # loss from other decoder layers
            num_dec_layer = 0
            for loss_rank_i, loss_bbox_i, loss_iou_i in zip(losses_rank[:-1],
                                                        losses_bbox[:-1],
                                                        losses_iou[:-1]):
                loss_dict[f'd{num_dec_layer}.loss_rank'] = loss_rank_i
                loss_dict[f'd{num_dec_layer}.loss_bbox'] = loss_bbox_i
    ...
    def loss_single(self,
        ...
        if self.rank_loss_type == 'RankSort' or self.rank_loss_type == 'BucketedRankSort':
            ...
            return ranking_loss * self.rank_loss_weight, sorting_loss * self.rank_loss_weight, losses_bbox * self.rank_loss_weight, losses_iou * self.rank_loss_weight

        elif self.rank_loss_type == 'APLoss' or self.rank_loss_type == 'BucketedAPLoss':
            losses_cls = self.loss_rank.apply(flat_preds, flat_labels, self.delta)
            losses_bbox = torch.sum(bbox_weights*loss_bbox)/bbox_avg_factor
            losses_iou = torch.sum(bbox_weights*loss_iou)/bbox_avg_factor
            return losses_cls * self.rank_loss_weight, losses_bbox, losses_iou       
