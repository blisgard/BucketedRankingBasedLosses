...
class RankBasedDeformableDETRHead(RankBasedDETRHead):
    ...
    def __init__(self,
        ...
        super(RankBasedDeformableDETRHead, self).__init__(
    ...
    def loss(self,
            ...
            if self.rank_loss_type == 'RankSort' or self.rank_loss_type == 'BucketedRankSort':
                ...
                enc_loss_cls, enc_losses_bbox, enc_losses_iou = \
                    self.loss_single(enc_cls_scores, enc_bbox_preds,
                                    gt_bboxes_list, binary_labels_list,
                                    img_metas, gt_bboxes_ignore)
                loss_dict['enc_loss_rank'] = enc_loss_cls
                loss_dict['enc_loss_bbox'] = enc_losses_bbox
                loss_dict['enc_loss_iou'] = enc_losses_iou
        ...
        if self.rank_loss_type == 'RankSort' or self.rank_loss_type == 'BucketedRankSort':
            ...
            loss_dict['loss_rank'] = losses_cls[-1]
            loss_dict['loss_bbox'] = losses_bbox[-1]
            loss_dict['loss_iou'] = losses_iou[-1]

            # loss from other decoder layers
            num_dec_layer = 0
            for loss_rank_i, loss_bbox_i, loss_iou_i in zip(losses_cls[:-1],
                                                        losses_bbox[:-1],
                                                        losses_iou[:-1]):
                loss_dict[f'd{num_dec_layer}.loss_rank'] = loss_rank_i
                loss_dict[f'd{num_dec_layer}.loss_bbox'] = loss_bbox_i
