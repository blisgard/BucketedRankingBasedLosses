...
from projects.models import RankBasedCoDeformDETRHead
from projects.models.query_denoising import build_dn_generator

EPS = 1e-12

@HEADS.register_module()
class RankBasedCoDINOHead(RankBasedCoDeformDETRHead):
    ...
    def __init__(self,
        ...
        super(RankBasedCoDINOHead, self).__init__(
    ...
    def loss(self,
        ...
        for loss_rank_i, loss_sort_i, loss_bbox_i, loss_iou_i in zip(losses_rank[:-1],
            ...
            loss_dict[f'd{num_dec_layer}.loss_sort'] = loss_sort_i
        ...
        if dn_cls_scores is not None:
            ...
            dn_losses_rank, dn_losses_sort, dn_losses_bbox, dn_losses_iou = self.loss_dn(
                dn_cls_scores, dn_bbox_preds, gt_bboxes_list, gt_labels_list,
                img_metas, dn_meta)
            # collate denoising loss
            loss_dict['dn_loss_rank'] = dn_losses_rank[-1]
            loss_dict['dn_loss_sort'] = dn_losses_sort[-1]
            loss_dict['dn_loss_bbox'] = dn_losses_bbox[-1]
            loss_dict['dn_loss_iou'] = dn_losses_iou[-1]
            num_dec_layer = 0
            for loss_rank_i, loss_sort_i, loss_bbox_i, loss_iou_i in zip(
                    dn_losses_rank[:-1],dn_losses_sort[:-1] ,dn_losses_bbox[:-1],
                    dn_losses_iou[:-1]):
                loss_dict[f'd{num_dec_layer}.dn_loss_rank'] = loss_rank_i
                loss_dict[f'd{num_dec_layer}.dn_loss_sort'] = loss_sort_i
    ...
    def loss_dn_single(self, dn_cls_scores, dn_bbox_preds, gt_bboxes_list,
        ...
        if len(cls_scores) > 0:
            ...
            IoU_targets = bbox_overlaps(pos_decode_bbox_pred, pos_decode_bbox_targets, is_aligned=True)
            flat_labels[flat_labels==1]=IoU_targets
            
            ranking_loss, sorting_loss = self.loss_rank.apply(flat_preds, flat_labels, 0.5)
        else:
            ranking_loss = torch.zeros(  # TODO: How to better return zero loss
                1,
                dtype=cls_scores.dtype,
                device=cls_scores.device)
            sorting_loss = torch.zeros(  # TODO: How to better return zero loss
                1,
                dtype=cls_scores.dtype,
                device=cls_scores.device)
